...
class TokensControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create token" do
    assert_difference('Token.count') do
      post tokens_url, params: { token: { amount: @token.amount, response: @token.response, status: @token.status } }
    end
  ...
  test "should update token" do
    patch token_url(@token), params: { token: { amount: @token.amount, response: @token.response, status: @token.status } }
